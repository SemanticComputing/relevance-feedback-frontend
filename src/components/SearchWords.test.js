...
it('renders the search words', () => {
  const tree = renderer
    .create(<SearchWords removeWord={removeWord} words={words} bannedWords={bannedWords} />)
    .toJSON();
...
it('calls removeWord when a word is clicked', () => {
  const resultElem = mount(<SearchWords removeWord={removeWord} words={words} bannedWords={bannedWords} />);
